...
message_sender_mixin.prototype._dispatch_responses = function(responses, next) {
	/*
		...
		next --> function
	*/
	if (toString.call(responses) !== '[object Array]') {
		throw TypeError('responses is not an array');
	}
	if (next && typeof next !== 'function') {
		throw TypeError('next is not a function');
	}
	...
	flow.serial(self, [
		...
		function(next) {
			for (var i in responses) {
				...
				if (!message_id) {
					self.err_cb(new TypeError('Invalid response from HPIT. No message id supplied in response'));
				}
				var response_payload = res['response'];
				if (!response_payload) {
					self.err_cb(new TypeError('Invalid response from HPIT. No response payload supplied'));
				}
...
message_sender_mixin.prototype.send = function(message_name, payload, callback, next) {
	...
	*/
	if (typeof message_name !== 'string') {
		throw new TypeError('message_id is not a string');
	}
	if (typeof payload !== 'string' && typeof payload !== 'object') {
		throw new TypeError('payload is not a string or a dict object');
	}
	...
	if (message_name === "transaction") {
		throw new Error("Cannot use message_name 'transaction'.  Use send_transaction() method for datashop transactions.");
	}
